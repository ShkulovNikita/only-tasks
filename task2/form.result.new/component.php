...
/*************************************************************************************************/
	...
	if ($arResult["ERROR"] == '')
	{
		...
		*/
		...
		// Заполнение информации о вопросах формы
		foreach ($arResult["arQuestions"] as $key => $arQuestion)
		{
			...
			// заполнить HTML-код, вставляемый в зависимости от типа поля
				...
				foreach ($arResult["arAnswers"][$FIELD_SID] as $key => $arAnswer)
				{
					...
					switch ($arAnswer["FIELD_TYPE"])
					{
						case "text":
							...
							choose_edit_function($FIELD_SID, $res);
...
// выбрать подходящую функцию для изменения HTML-кода поля ввода
function choose_edit_function($field_id, &$field_html)
{
	switch ($field_id)
	{
		case "medicine_name":
			edit_text_field($field_id, $field_html);
			break;
		case "medicine_company":
			edit_text_field($field_id, $field_html);
			break;
		case "medicine_phone":
			edit_phone_field($field_id, $field_html);
			break;
		case "medicine_email":
			edit_email_field($field_id, $field_html);
			break;
		case "medicine_message":
			edit_textarea_field($field_id, $field_html);
			break;
	}
}

function edit_phone_field($field_id, &$field_html)
{
	$field_html = preg_replace(
		'#<input type="text"#', 
		'<input type="tel" class="input__input" id="' . $field_id . '" ' .
		'data-inputmask=' . '"' . "'mask': '+79999999999', 'clearIncomplete': 'true'" . '" ' .
		'maxlength="12" x-autocompletetype="phone-full" required=""',
		$field_html
	);
}

function edit_text_field($field_id, &$field_html) 
{
	$field_html = preg_replace(
		'#<input type="text"#',
		'<input type="text" class="input__input" id="' . $field_id . '" required=""',
		$field_html
	);
}

function edit_email_field($field_id, &$field_html)
{
	$field_html = preg_replace(
		'#<input type="text"#',
		'<input type="email" class="input__input" id="' . $field_id . '" required=""',
		$field_html
	);
}

function edit_textarea_field($field_id, &$field_html)
{
	$field_html = preg_replace(
		'#<input type="text"#',
		'<textarea class="input__input" type="text" id="' . $field_id . '"',
		$field_html
	);
	$field_html = preg_replace(
		'#value="">#',
